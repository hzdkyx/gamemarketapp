...
const user = (role: UserRecord["role"], allowRevealSecrets = false): UserRecord => ({
  id: `${role}-1`,
  name: role,
  username: role,
  role,
...
describe("auth permissions", () => {
  it("grants full local permissions to admin", () => {
    expect(Object.values(getPermissionsForUser(user("admin"))).every(Boolean)).toBe(true);
  });

  it("allows operators to edit operations and reveal secrets only when enabled", () => {
    expect(getPermissionsForUser(user("operator")).canRevealSecrets).toBe(false);
    expect(getPermissionsForUser(user("operator", true)).canRevealSecrets).toBe(true);
